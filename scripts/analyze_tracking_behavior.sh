...
RUN_DIR="${1:?usage: [MODE=actuated-2eye-rppo] analyze_tracking_behavior.sh RUN_DIR [Hydra overrides ...]}"
shift
MODE="${MODE:-actuated-2eye-rppo}"
EPISODES="${EPISODES:-50}"
OUTPUT_DIR="${OUTPUT_DIR:-${RUN_DIR}/behavior_analysis}"
...
case "${MODE}" in
  ...
  *)
    ...
    echo "Expected one of: original-3eye-mlp, fixed-2eye-mlp, actuated-2eye-mlp, fixed-2eye-rppo, actuated-2eye-rppo" >&2
    exit 2
    ;;
esac
...
exec "${PYTHON_BIN}" -m cambrian.analysis.tracking_behavior \
  --episodes "${EPISODES}" \
  ...
  "logdir=${RUN_DIR}" \
  "expdir=${RUN_DIR}" \
  +eval_env.renderer.save_mode=NONE \
  "$@"
