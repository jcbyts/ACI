...
EXPNAME="${EXPNAME:-exp_frozen2_goal_moving_ground_smallcnn_relative_respawn_150k}"
RENDER="${RENDER:-true}"
SAVE_MODE="${SAVE_MODE:-MP4}"
GOAL_RESPAWN_REWARD="${GOAL_RESPAWN_REWARD:-10.0}"
...
POLICY_KWARGS="${POLICY_KWARGS:-small_cnn_shared}"
...
"${PYTHON_BIN}" -m cambrian.main --train \
  ...
  trainer.callbacks.eval_callback.callback_after_eval.callbacks.stop_training_on_reward_threshold_callback.reward_threshold=50 \
  trainer.max_episode_steps=256 \
  ...
  +env.reward_fn.reward_if_goal_respawned.reward="${GOAL_RESPAWN_REWARD}" \
